...
const AdminStateWiseList = () => {
	...
	function editAction(i) {
		...
		let date = currentDiv.parentElement.parentElement.parentElement.children[5].innerHTML;
		let type = currentDiv.parentElement.parentElement.parentElement.children[6].innerHTML;
		localStorage.setItem("societyNameSociety", nameSociety);
		localStorage.setItem("societyAddress", address);
		localStorage.setItem("societyState", state);
		localStorage.setItem("societyDistrict", district);
		localStorage.setItem("societyDate", date);
		localStorage.setItem("societyType", type);
			...
			<div id="mainContainer" className="pl-[6%] pt-[5rem] p-4 transition-all duration-300 bg-primary-100">
				<div className="p-4 w-[80%] mx-auto">
					<ul className="grid grid-cols-2 w-[40%] text-center overflow-hidden">
						<TabNavItem title="Society Form" id="tab1" activeTab={activeTab} setActiveTab={setActiveTab} />
					...
					<div>
						<TabContent id="tab1" activeTab={activeTab}>
							<div className="border-2 border-primary-800 bg-white p-4 w-full rounded-lg rounded-tl-none">
								...
								<div className="grid grid-cols-2 gap-5 mt-3">
									<div className="w-full relative">
										...
										<span className="relative">
											...
											<input type="text" placeholder="Name of society" data-title="societyNameSociety" className="my-3 form-input outline-none peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9 placeholder:font-light hover:border-primary-700 focus:border-primary" />
										</span>
									</div>
									<div className="w-full">
										<p className="text-black">Select File</p>
										<div className="relative my-3 form-input outline-none peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9 placeholder:font-light hover:border-primary-700 focus:border-primary cursor-pointer">
											<p className="text-black truncate cursor-pointer" id="fileInputName">
												Select File
											</p>
											<span className="pointer-events-none absolute left-0 top-0 flex h-full w-10 items-center justify-center text-primary-700 peer-focus:text-primary">
											...
											<input
												type="file"
												className="absolute top-1/2 -translate-y-1/2 left-0 opacity-0 cursor-pointer"
												id="fileInput"
												onInputCapture={() => {
								...
								<div className="my-3">
									...
									<textarea rows="3" placeholder="Your Address (Area, Street and PIN Code)" data-title="societyAddress" className="form-textarea outline-none mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent p-2.5 placeholder:font-light hover:border-primary-700 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"></textarea>
								</div>
								...
								<div className="grid grid-cols-3 gap-5 my-3 mb-5">
									<div className="w-full">
										<p className="text-black">Enter Date of Registration</p>
										<span className="relative">
											...
											<input type="date" placeholder="DD/MM/YYYY" data-title="societyDate" className="my-3 form-input outline-none peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9 placeholder:font-light hover:border-primary-700 focus:border-primary" />
										</span>
									</div>
									<div className="w-full">
										<p className="text-black">Enter Area of Operation</p>
										<span className="relative">
											<span className="pointer-events-none absolute left-2 top-0 flex  items-center justify-center text-primary-700 peer-focus:text-primary">
												<svg fill="none" width={20} height={20} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
													<path d="M18 16.0156C19.2447 16.5445 20 17.2392 20 18C20 19.6568 16.4183 21 12 21C7.58172 21 4 19.6568 4 18C4 17.2392 4.75527 16.5445 6 16.0156" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
													<path d="M17 8.44444C17 11.5372 12 17 12 17C12 17 7 11.5372 7 8.44444C7 5.35165 9.23858 3 12 3C14.7614 3 17 5.35165 17 8.44444Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
													<circle cx={12} cy={8} r={1} stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
												</svg>
											</span>
											<input type="text" placeholder="Area of Operation" data-title="societyAddress" className="my-3 form-input outline-none peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9 placeholder:font-light hover:border-primary-700 focus:border-primary" />
										</span>
									</div>
									<div className="w-full">
										<p className="text-black">Select Sector Type</p>
										<span className="relative flex">
											<select data-title="societyType" id="SocietyType" className="my-3 form-input outline-none peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9 placeholder:font-light hover:border-primary-700 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent">
												<option selected disabled>
													--Select Type--
												</option>
						...
						<TabContent id="tab2" activeTab={activeTab}>
							<div className="border-2 border-primary-800 bg-gradient-to-br from-primary-200 to-primary-500 p-4 w-full rounded-lg">
								<Table editAction={editAction} deleteAction={deleteAction} rowLimit={25} link={process.env.REACT_APP_SERVER + "/api/search-record/"} admin={true} columns={["Sr No", "Name of the Society", "Address", "State/UT", "District Name", "Date", "Sector Type", "Action"]} classes={"p-4 py-3 w-full mx-auto mt-5 overflow-x-scroll noScrollBar"} />
							</div>
						</TabContent>
					</div>
				</div>
