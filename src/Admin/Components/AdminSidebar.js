...
const AdminSidebar = () => {
	const [isActiveMenu, setisActiveMenu] = useState("abc");

	const data = [
		{
			name: "Home",
			svg: (
				<svg className="min-h-7 !h-7 min-w-7 !w-7 text-black group-hover:text-primary-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
		...
		},
		{
			name: "Society",
			svg: (
				<svg className="min-h-7 !h-7 min-w-7 !w-7 text-black group-hover:text-primary-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			link: "/admin/society-list",
		},
		{
			name: "User",
			svg: (
				<svg className="h-7 w-7 text-black group-hover:text-primary-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		},
		{
			...
			link: "/admin/recieved-applications",
		},
		{
			name: "Liquidation",
			svg: (
	...
	if (isActiveMenu === "abc") {
		if (window.location.pathname.toString() === "/admin") {
			setisActiveMenu("Home");
		} else if (window.location.pathname.toString() === "/admin/society-list") {
			setisActiveMenu("Society");
		} else if (window.location.pathname.toString() === "/admin/user-list") {
			setisActiveMenu("User");
		} else if (window.location.pathname.toString() === "/admin/recieved-applications") {
			setisActiveMenu("Application");
		} else if (window.location.pathname.toString() === "/admin/liquidation-list") {
			setisActiveMenu("Liquidation");
		} else if (window.location.pathname.toString() === "/admin/bank-list") {
			setisActiveMenu("Bank");
		} else if (window.location.pathname.toString() === "/admin/settings") {
			setisActiveMenu("Setting");
		} else {
			setisActiveMenu("#");
		}
	}
	...
	return (
		<div className="fixed top-0 left-0 bg-gradient-to-b from-primary-100 to-primary-200 h-screen w-[5%] px-2 z-50 shadow-xl overflow-hidden transition-all duration-300" id="sidebarContainer">
			<div className="flex h-full w-full flex-col items-center" id="sidebarUpperLinks">
				<div className="flex pt-4">
					<a href="/admin">
						<img className="h-11 w-11 transition-transform duration-500 ease-in-out hover:rotate-[360deg]" src="/logo.png" alt="logo" />
					</a>
				</div>

				<div className="is-scrollbar-hidden flex grow flex-col items-center w-full space-y-4 overflow-y-auto pt-6">
					{data.map((item, index) => (
						<a href={item.link} key={index} className={"menuLink flex justify-center h-11 min-w-11 w-full gap-2 pl-2 items-center rounded-lg text-primary hover:bg-white outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 " + (isActiveMenu === item.name ? "bg-white" : "")}>
							{item.svg}
							<p className={"menuTitle text-primary-500 rounded-lg w-0 overflow-hidden " + (isActiveMenu === item.name ? "font-bold" : "")}>{item.name}</p>
						</a>
					))}
				</div>

				<div className="flex flex-col items-center w-full space-y-3 py-3" id="sidebarLowerLinks">
					<a href="/admin/settings" className={"menuLink flex justify-center h-11 min-w-11 w-full gap-2 pl-2 items-center rounded-lg text-primary hover:bg-white outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 " + (isActiveMenu === "Setting" ? "bg-white" : "")}>
						<svg className="h-7 w-7" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
						...
						<p className={"menuTitle text-primary-500 rounded-lg w-0 overflow-hidden " + (isActiveMenu === "Setting" ? "font-bold" : "")}>Setting</p>
					</a>

					<div className="flex">
						<button className="avatar h-12 w-12 relative">
							<img className="rounded-full" src="https://lineone.piniastudio.com/images/avatar/avatar-12.jpg" alt="avatar" />
							<span className="absolute right-0 top-0 h-3.5 w-3.5 rounded-full border-2 border-white bg-green-500 dark:border-navy-700"></span>
						</button>
					</div>
				</div>
			</div>
